    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(huodongYuyue !=null){
            ...
            if(huodong != null){
            BeanUtils.copyProperties( huodong , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
    ...
    */
    ...
    public R shenhe(@RequestBody HuodongYuyueEntity huodongYuyueEntity, HttpServletRequest request){
        ...
        if(huodongYuyueEntity.getHuodongYuyueYesnoTypes() == 2){//通过
            HuodongEntity huodongEntity = huodongService.selectById(oldHuodongYuyue.getHuodongId());
            if(huodongEntity.getHuodongKucunNumber() <= 0){
                return R.error("可报名人数不足");
            }
            huodongEntity.setHuodongKucunNumber(huodongEntity.getHuodongKucunNumber()-1);
            huodongService.updateById(huodongEntity);
        }
        ...
        HuodongYuyueEntity huodongYuyue = huodongYuyueService.selectById(id);
            if(huodongYuyue !=null){
                ...
                //级联表
                    HuodongEntity huodong = huodongService.selectById(huodongYuyue.getHuodongId());
                if(huodong != null){
                    BeanUtils.copyProperties( huodong , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setHuodongId(huodong.getId());
                }
                //级联表
                    YonghuEntity yonghu = yonghuService.selectById(huodongYuyue.getYonghuId());
